...
class ChatReplayDownloader:
    ...
    _YT_REGEX = r'(?:/|%3D|v=|vi=)([0-9A-z-_]{11})(?:[%#?&]|$)'
    _YOUTUBE_API_BASE_TEMPLATE = 'https://www.youtube.com/{}/{}?continuation={}&pbj=1&hidden=false'
    _YOUTUBE_API_PARAMETERS_TEMPLATE = '&playerOffsetMs={}'

    _TWITCH_REGEX = r'(?:/videos/|/v/)(\d+)'
    _TWITCH_CLIENT_ID = 'kimne78kx3ncx6brgo4mv6wki5h1ko'  # public client id
    _TWITCH_API_TEMPLATE = 'https://api.twitch.tv/v5/videos/{}/comments?client_id={}'
    ...
    # used for debugging
    _TYPES_OF_KNOWN_MESSAGES = []
    for key in _TYPES_OF_MESSAGES:
        _TYPES_OF_KNOWN_MESSAGES.extend(_TYPES_OF_MESSAGES[key])
    ...
    def _session_get_json(self, url):
        return self._session_get(url).json()
    ...
    def _arbg_int_to_rgba(self, argb_int):
        red = (argb_int >> 16) & 255
    ...
    def _rgba_to_hex(self, colours):
        return '#{:02x}{:02x}{:02x}{:02x}'.format(*colours)

    def _get_colours(self, argb_int):
        rgba_colour = self._arbg_int_to_rgba(argb_int)
        hex_colour = self._rgba_to_hex(rgba_colour)
        return {
            'rgba': rgba_colour,
            'hex' : hex_colour
        }
    ...
    def _get_initial_youtube_info(self, video_id):
    ...
    def _get_replay_info(self, continuation, offset_microseconds):
        url = self._YOUTUBE_API_BASE_TEMPLATE.format(
            'live_chat_replay', 'get_live_chat_replay', continuation) + self._YOUTUBE_API_PARAMETERS_TEMPLATE.format(
            offset_microseconds)
    ...
    def _ensure_seconds(self, time, default=0):
        try:
            return int(time)
        except ValueError:
            return self._time_to_seconds(time)
        except:
            return default
    ...
    def _parse_item(self, item):
        ...
        if index not in self._TYPES_OF_KNOWN_MESSAGES:
            pass
        ...
        if 'timestamp' in data:
            ...
            data['time_in_seconds'] = int(
                self._time_to_seconds(data['time_text']))
        else:
            data['timestamp'] = None
    ...
    def get_youtube_messages(self, video_id, start_time=0, end_time=None, message_type='messages'):
        start_time = self._ensure_seconds(start_time, 0)
        end_time = self._ensure_seconds(end_time, None)

        messages = []

        offset_milliseconds = start_time * 1000 if start_time > 0 else 0

        continuation_by_title_map = self._get_initial_youtube_info(video_id)
        ...
        try:
            while True:
                ...
                for action in actions:
                    ...
                    elif (message_type != 'superchat' and index in self._TYPES_OF_MESSAGES['superchat_message'] +
                          self._TYPES_OF_MESSAGES['superchat_ticker']):
                        continue
    ...
    def get_twitch_messages(self, video_id, start_time=0, end_time=None):
        ...
        messages = []
        api_url = self._TWITCH_API_TEMPLATE.format(
            video_id, self._TWITCH_CLIENT_ID)
                ...
                for comment in info['comments']:
                    ...
                    data = {
                        'timestamp'      : int(datetime.datetime.strptime(
                            created_at,
                            '%Y-%m-%dT%H:%M:%S.%fZ' if '.' in created_at else '%Y-%m-%dT%H:%M:%SZ').timestamp() * 1e6),
                        'time_text'      : self._seconds_to_time(int(time_in_seconds)),
